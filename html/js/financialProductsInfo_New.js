...
require(['jquery','publicJS','swal'],function($,publicJS,swal) {
	...
	$(".l_btn").on("click",function() {
		...
		// 	return false;
		// }
		if ($(this).hasClass("l_dingxiangBtn")) {
			alert("校验定向密码");
			if (true) {
				alert("校验成功")
				$(this).removeClass("l_dingxiangBtn").text("立即投标")
				$(".l_inputPassword").animate({bottom:-8+"rem"},600);;
			}
	...
	$("#inputPassword").on("input",function() {
		...
		$(".l_inputPassword ul li").each(function(index) {
				if (index<leg) {
					$(this).addClass("l_pwdDot")
				}else{
					$(this).removeClass("l_pwdDot")
				}
		})
	})
